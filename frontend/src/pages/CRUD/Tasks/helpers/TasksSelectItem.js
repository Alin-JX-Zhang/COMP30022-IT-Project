...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/tasks/autocomplete`, { params });
  return response.data;
}

const TasksSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.tasks.hasPermissionToCreate,
});

export default connect(select)(TasksSelectItem);
