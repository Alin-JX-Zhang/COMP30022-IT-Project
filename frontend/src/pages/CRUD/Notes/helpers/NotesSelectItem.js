...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/notes/autocomplete`, { params });
  return response.data;
}

const NotesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.notes.hasPermissionToCreate,
});

export default connect(select)(NotesSelectItem);
