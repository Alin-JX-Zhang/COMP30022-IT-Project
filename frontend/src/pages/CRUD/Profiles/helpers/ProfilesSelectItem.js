...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/profiles/autocomplete`, { params });
  return response.data;
}

const ProfilesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.profiles.hasPermissionToCreate,
});

export default connect(select)(ProfilesSelectItem);
